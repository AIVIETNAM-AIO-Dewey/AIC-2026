...
def base_frame(payload: dict[str, Any], *, score: float, rank: int, score_type: str) -> dict[str, Any]:
    ...
    return {
        ...
        "image_relpath": str(payload.get("image_relpath") or f"keyframes/{video_id}/{frame_idx:08d}.jpg"),
...
class QdrantHttpClient:
    ...
    def query(self, collection: str, vector_name: str, vector: np.ndarray, limit: int, query_filter: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        body: dict[str, Any] = {
    ...
    def retrieve(self, collection: str, ids: list[int]) -> dict[int, dict[str, Any]]:
        if not ids:
            return {}
        result = self._post(f"/collections/{collection}/points", {"ids": ids, "with_payload": True, "with_vector": False})
    ...
    def dam_for_frame(self, video_id: str, frame_idx: int) -> list[dict[str, Any]]:
        result = self._post(
            ...
            {"filter": {"must": [{"key": "video_id", "match": {"value": video_id}}, {"key": "frame_idx", "match": {"value": frame_idx}}]}, "limit": 100, "with_payload": True, "with_vector": False},
        )
