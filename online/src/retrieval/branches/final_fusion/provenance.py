...
def _compact_region_evidence(value: Any) -> dict[str, Any] | None:
    ...
    return _pick(
        value,
        (
            "point_id", "region_id", "class_entity", "description_en", "bbox",
            "query_role", "best_query_role", "query_language", "best_query_language",
            "rank", "bm25_raw", "lse_score",
        ),
    )
...
def compact_branch_evidence(branch: str, item: dict[str, Any]) -> dict[str, Any]:
    ...
    if branch == "branch1":
        result = _pick(
            item,
            (
                "final_score", "score", "score_type", "best_model", "best_stream_rank",
                "best_query_role", "best_query_language",
            ),
        )
    ...
    if branch == "branch2":
        result = _pick(
            item,
            (
                "dense_raw", "dense_normalized", "dense_observed",
                "sparse_raw", "sparse_normalized", "sparse_observed", "sparse_bm25_raw",
                "hybrid_score", "reranked_score", "score", "score_type", "hybrid_rank",
                "pre_rerank_rank", "dam_winner", "dense_rank",
                "sparse_rank", "dense_best_query_role", "sparse_best_query_role",
                "beit3_raw_cosine", "beit3_normalized", "beit3_best_query_role",
                "sparse_winner",
            ),
        )
        ...
        if beit_observed:
            result["beit3_best_query_language"] = str(
                item.get("beit3_best_query_language") or "en"
            )
        else:
            for field in (
                "beit3_raw_cosine", "beit3_normalized", "beit3_best_query_role",
                "beit3_query_scores", "beit3_best_query_language",
            ):
    ...
    if branch == "ocr":
        return _pick(
            item,
            (
                "ocr_text", "full_text", "ocr_raw_score", "ocr_combined_score",
                "ocr_normalized_score", "ocr_best_query_role", "ocr_best_query_language",
                "ocr_best_rank", "bm25_raw", "bm25_relevance", "token_coverage",
                "ngram_coverage", "adjacent_bigram_coverage", "matched_terms",
            ),
        )
    if branch == "asr":
        return _pick(
            item,
            (
                "asr_text", "asr_transcript", "transcript", "asr_raw_score", "asr_combined_score",
                "asr_normalized_score", "asr_best_query_role", "asr_best_query_language",
                "asr_best_rank", "bm25_raw", "bm25_relevance", "token_coverage",
                "ngram_coverage", "adjacent_bigram_coverage", "matched_terms",
                # Segment identity/time are part of the winning ASR evidence.
                "asr_segment_id", "segment_id", "asr_start_s", "asr_end_s",
                "segment_start_s", "segment_end_s", "start_s", "end_s",
            ),
        )
...
_CANONICAL_FIELDS = (
    "frame_uid", "point_id", "global_idx", "video_id", "frame_idx", "keyframe_n",
    "pts_time_s", "fps", "image_relpath", "submission_string",
)
_RRF_FIELDS = (
    "rank", "pre_rerank_rank", "rrf_score", "branch_agreement_count",
    "observed_branches", "branch_ranks", "rrf_contributions",
    "branch_normalized_scores", "weighted_normalized_score", "best_branch_rank",
)
_RERANK_FIELDS = (
    "score", "score_type", "final_score",
    "beit3_raw_cosine", "beit3_normalized", "rrf_normalized",
    "beit3_best_query_role", "beit3_best_query_language", "beit3_query_scores",
    "rank_delta", "rerank_formula",
)
...
def materialize_fusion_candidate(
    ...
    *,
    ...
    fields = _CANONICAL_FIELDS + _RRF_FIELDS + (
