...
def print_fusion_table(
    query_title: str, fused_results: list[dict[str, Any]], weights: dict[str, float]
):
    print(f"\n{'=' * 105}")
    print(f"🏆 PHASE 3 STAGE-1 FUSED RESULTS: {query_title}")
    print(
        f"⚖️ Active Channel Weights: Vis={weights.get('vis', 0.40):.2f}, DAM={weights.get('dam', 0.40):.2f}, ASR={weights.get('asr', 0.20):.2f}, OCR={weights.get('ocr', 0.00):.2f} | RRF Constant k=60"
    )
    print(f"{'=' * 105}")
...
def main():
    ...
    queries = [
        (
            "Test 1 (VQA - Cooking Class / Recipe)",
            "VQA",
        ...
        (
            "Test 2 (KIS - Bicycle Road Race Flycam)",
            "KIS",
