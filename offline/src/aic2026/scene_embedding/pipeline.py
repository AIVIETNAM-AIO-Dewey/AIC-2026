"""Batched SigLIP2 embedding of the keyframes named by a frame manifest."""
...
def embed_frames(
    *,
    ...
    """Embed one video and publish its index plus matrix, or nothing at all.

    A frame costs milliseconds here rather than the seconds DAM needs, so there is
    no intra-shard `.partial`: an interrupted video is recomputed, and `--resume`
    works at video granularity through the run manifest.
    """
    if batch_size < 1:
    ...
    # Publish the matrix first: the index is the commit point, so a crash between the
    # two leaves an orphan matrix that the runner detects rather than a dangling index.
