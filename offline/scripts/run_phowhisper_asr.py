...
REPOSITORY_ROOT = OFFLINE_ROOT.parent
...
def main(argv: list[str] | None = None) -> int:
    ...
    config_path = args.config or (OFFLINE_ROOT / "configs" / "offline" / "asr.yaml")
    ...
    output_root = args.output_root or Path(
        os.environ.get("AIC_ARTIFACT_ROOT", REPOSITORY_ROOT / "artifacts")
    )
    output_dir = output_root / "asr_segments"
    output_dir.mkdir(parents=True, exist_ok=True)

    data_root = args.data_root or Path(os.environ.get("AIC_DATA_ROOT", REPOSITORY_ROOT))
