...
def test_committed_notebooks_are_thin_and_cleared() -> None:
    notebooks = sorted((OFFLINE_ROOT / "notebooks").glob("*.ipynb"))
    assert notebooks
...
def test_cloud_runtime_mirrors_core_dependency_pins() -> None:
    ...
    runtime_lines = {
        line.strip()
        for line in (OFFLINE_ROOT / "requirements" / "runtime-base.txt")
...
def test_importable_package_has_no_cloud_specific_absolute_paths() -> None:
    ...
    for path in (OFFLINE_ROOT / "src").rglob("*.py"):
...
def test_markdown_fences_and_relative_links_are_valid() -> None:
    ...
    for directory in ("docs", "data", "artifacts", "runs", "reports", ".github"):
        paths.extend((REPO_ROOT / directory).rglob("*.md"))
    paths.extend((OFFLINE_ROOT / "tests").rglob("*.md"))
    for path in paths:
        text = path.read_text(encoding="utf-8")
